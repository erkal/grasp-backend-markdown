...
type Meaning
    ...
    | SoftLineBreakToken
    | HardLineBreakToken
    | WikilinkOpenToken Bool -- isEmbed
    | WikilinkCloseToken
...
findWikilinkOpenTokens : String -> List Token
findWikilinkOpenTokens str =
    Regex.find wikilinkOpenTokenRegex str
        |> List.filterMap regMatchToWikilinkOpenToken


wikilinkOpenTokenRegex : Regex
wikilinkOpenTokenRegex =
    Regex.fromString "(\\\\*)(\\!)?(\\[\\[)"
        |> Maybe.withDefault Regex.never


regMatchToWikilinkOpenToken : Regex.Match -> Maybe Token
regMatchToWikilinkOpenToken regMatch =
    case regMatch.submatches of
        maybeBackslashes :: maybeEmbed :: (Just _) :: _ ->
            let
                backslashesLength : Int
                backslashesLength =
                    Maybe.map String.length maybeBackslashes
            ...
            if not (isEven backslashesLength) then
                ...
                Just
                    { index = index
                    , length = length
                    , meaning = WikilinkOpenToken isEmbed
                    }
...
findWikilinkCloseTokens : String -> List Token
findWikilinkCloseTokens str =
    Regex.find wikilinkCloseTokenRegex str
        |> List.filterMap regMatchToWikilinkCloseToken


wikilinkCloseTokenRegex : Regex
wikilinkCloseTokenRegex =
    Regex.fromString "(\\\\*)(\\]\\])"
        |> Maybe.withDefault Regex.never


regMatchToWikilinkCloseToken : Regex.Match -> Maybe Token
regMatchToWikilinkCloseToken regMatch =
    case regMatch.submatches of
...
-- Wikilink Tokens To Matches


wikilinkTTM : ( List Token, Parser ) -> Parser
wikilinkTTM ( tokens, model ) =
    case tokens of
        [] ->
            reverseTokens model
        ...
        token :: tokensTail ->
            case token.meaning of
                WikilinkCloseToken ->
                    model.tokens
                        |> findToken isWikilinkOpenToken
                        |> Maybe.map
                            (wikilinkToMatch token tokensTail model)
                        |> Maybe.withDefault
...
isWikilinkOpenToken : Token -> Bool
isWikilinkOpenToken token =
    case token.meaning of
        WikilinkOpenToken _ ->
            True
...
wikilinkToMatch :
    ...
    -> ( List Token, Parser )
wikilinkToMatch closeToken tokensTail model ( openToken, _, remainTokens ) =
    let
        isEmbed : Bool
        isEmbed =
            case openToken.meaning of
                WikilinkOpenToken embed ->
                    embed
        ...
        wikilinkData =
            Wikilink.parseContent isEmbed rawContent

        match : Match
        match =
            Match
                { type_ = WikilinkType wikilinkData
...
walk function (Inline inline) =
    case inline.content of
        Link url maybeTitle inlines ->
            Inline { inline | content = Link url maybeTitle (List.map (walk function) inlines) }
                |> function

        Image url maybeTitle inlines ->
            Inline { inline | content = Image url maybeTitle (List.map (walk function) inlines) }
                |> function

        HtmlInline tag attrs inlines ->
            Inline { inline | content = HtmlInline tag attrs (List.map (walk function) inlines) }
                |> function

        Emphasis length inlines ->
            Inline { inline | content = Emphasis length (List.map (walk function) inlines) }
                |> function

        Custom custom inlines ->
            Inline { inline | content = Custom custom (List.map (walk function) inlines) }
                |> function
...
query function ((Inline inline) as wrappedInline) =
    case inline.content of
        Link _ _ inlines ->
            function wrappedInline ++ List.concatMap (query function) inlines
